(function(a1, aE) {
	...
	(function(dc, ch) {
		var cw, df, cc, cm, cF, cI, cT, dj, cG, cW, cA, cn, c5, c0, dd, cb, cD, c7 = "sizzle" + -(new Date()),
			...
			b3 = "(?:\\\\.|[\\w-]|[^\\x00-\\xa0])+",
			cJ = b3.replace("w", "w#"),
			c3 = "\\[" + co + "*(" + b3 + ")" + co + "*(?:([*^jq|!~]?=)" + co + "*(?:(['\"])((?:\\\\.|[^\\\\])*?)\\3|(" + cJ + ")|)|)" + co + "*\\]",
			cj = ":(" + b3 + ")(?:\\(((['\"])((?:\\\\.|[^\\\\])*?)\\3|((?:\\\\.|[^\\\\()[\\]]|" + c3.replace(3, 8) + ")*)|.*)\\)|)",
			cq = new RegExp("^" + co + "+|((?:^|[^\\\\])(?:\\\\.)*)" + co + "+jq", "g"),
			ct = new RegExp("^" + co + "*," + co + "*"),
			...
			cZ = {
				...
				bool: new RegExp("^(?:" + b5 + ")jq", "i"),
				needsContext: new RegExp("^" + co + "*[>+~]|:(even|odd|eq|gt|lt|nth|first|last)(?:\\(" + co + "*((?:-\\d)?\\d*)" + co + "*\\)|)(?=[^-]|jq)", "i")
			},
			cN = /^[^{]+\{\s*\[native \w/,
			cP = /^(?:#([\w-]+)|(\w+)|\.([\w-]+))jq/,
			b9 = /^(?:input|select|textarea|button)jq/i,
			ck = /^h\djq/i,
			cL = /'|\\/g,
		...
		function cu(ds, dk, dw, dy) {
			...
			return de(ds.replace(cq, "jq1"), dk, dw, dy)
		}
		...
		cm = cu.selectors = {
			...
			filter: {
				...
				CLASS: function(e) {
					var dk = b6[e + " "];
					return dk || (dk = new RegExp("(^|" + co + ")" + e + "(" + co + "|jq)")) && b6(e, function(dl) {
				...
				ATTR: function(dl, dk, e) {
					return function(dn) {
						...
						dm += "";
						return dk === "=" ? dm === e : dk === "!=" ? dm !== e : dk === "^=" ? e && dm.indexOf(e) === 0 : dk === "*=" ? e && dm.indexOf(e) > -1 : dk === "jq=" ? e && dm.slice(-e.length) === e : dk === "~=" ? (" " + dm + " ").indexOf(e) > -1 : dk === "|=" ? dm === e || dm.slice(0, e.length + 1) === e + "-" : false
					}
				},
				...
				if ((dn = cm.relative[dq[dm].type])) {
					...
					if (dn[c7]) {
						...
						return ce(dm > 1 && di(e), dm > 1 && cg(dq.slice(0, dm - 1).concat({
							value: dq[dm - 2].type === " " ? "*" : ""
						})).replace(cq, "jq1"), dn, dm < dl && c8(dq.slice(dm, dl)), dl < dp && c8((dq = dq.slice(dl))), dl < dp && cg(dq))
					}
					e.push(dn)
				}
			}
	...
	function bx(b5, b4, b6) {
		if (b6 === aE && b5.nodeType === 1) {
			var b3 = "data-" + b4.replace(aL, "-jq1").toLowerCase();
	...
	var a7, bZ, bL = /[\t\r\n\f]/g,
		ah = /\r/g,
		aD = /^(?:input|select|textarea|button|object)jq/i,
		C = /^(?:a|area)jq/i,
		ao = /^(?:checked|selected)jq/i,
	...
	var bG = /^(?:input|select|textarea)jq/i,
		a2 = /^key/,
		bM = /^(?:mouse|contextmenu)|click/,
		bA = /^(?:focusinfocus|focusoutblur)jq/,
		bt = /^([^.]*)(?:\.(.+)|)jq/;
	...
	bI.event = {
		...
		trigger: function(b3, ca, b6, ch) {
			...
			b3.namespace_re = b3.namespace ? new RegExp("(^|\\.)" + b4.join("\\.(?:.*\\.|)") + "(\\.|jq)") : null;
			b3.result = aE;
			if (!b3.target) {
				b3.target = b6
			}
	...
	var d = "abbr|article|aside|audio|bdi|canvas|data|datalist|details|figcaption|figure|footer|header|hgroup|mark|meter|nav|output|progress|section|summary|time|video",
		...
		q = /^(?:checkbox|radio)jq/i,
		bT = /checked\s*(?:[^=]|=\s*.checked.)/i,
		by = /^jq|\/(?:java|ecma)script/i,
		ap = /^true\/(.*)/,
		aI = /^\s*<!(?:\[CDATA\[|--)|(?:\]\]|--)>\s*jq/g,
		R = {
				...
				if (b7 || b7 === 0) {
					if (bI.type(b7) === "object") {
						...
						if (!H.test(b7)) {
							...
							b4 = R[cg] || R._default;
							ce.innerHTML = b4[1] + b7.replace(aB, "<jq1></jq2>") + b4[2];
							cb = b4[0];
							while (cb--) {
								ce = ce.lastChild
							}
	...
	var aC, bn, D, bf = /alpha\([^)]*\)/i,
		aR = /opacity\s*=\s*([^)]*)/,
		bm = /^(top|right|bottom|left)jq/,
		E = /^(none|table(?!-c[ea]).+)/,
		aW = /^margin/,
		a8 = new RegExp("^(" + bz + ")(.*)jq", "i"),
		U = new RegExp("^(" + bz + ")(?!px)[a-z%]+jq", "i"),
		Q = new RegExp("^([+-])=(" + bz + ")", "i"),
		bi = {
			BODY: "block"
		},
	...
	if (!bI.support.opacity) {
		bI.cssHooks.opacity = {
			get: function(b3, e) {
				return aR.test((e && b3.currentStyle ? b3.currentStyle.filter : b3.style.filter) || "") ? (0.01 * parseFloat(RegExp.jq1)) + "" : e ? "1" : ""
			},
			set: function(b6, b7) {
				var b5 = b6.style,
					b3 = b6.currentStyle,
	...
	var bu = /%20/g,
		aP = /\[\]jq/,
		S = /\r?\n/g,
		c = /^(?:submit|button|image|reset|file)jq/i,
	...
	var b1, W, bN = bI.now(),
		...
		M = /([?&])_=[^&]*/,
		ae = /^(.*?):[ \t]*([^\r\n]*)\r?jq/mg,
		A = /^(?:about|app|app-storage|.+-extension|file|res|widget):jq/,
		o = /^(?:GET|HEAD)jq/,
		aF = /^\/\//,
		aS = /^([\w.+-]+:)(?:\/\/([^\/?#:]*)(?::(\d+)|)|)/,
		b0 = bI.fn.load,
	...
	bI.extend({
		...
		ajax: function(b7, b4) {
			...
			if (!cb.hasContent) {
				...
				if (cb.cache === false) {
					cb.url = M.test(b8) ? b8.replace(M, "jq1_=" + bN++) : b8 + (ax.test(b8) ? "&" : "?") + "_=" + bN++
				}
			}
			if (cb.ifModified) {
	...
	var J, ab, bQ = /^(?:toggle|show|hide)jq/,
		bJ = new RegExp("^(?:([+-])=|)(" + bz + ")([a-z%]*)jq", "i"),
		bP = /queueHooksjq/,
		az = [h],
		aY = {
			"*": [function(e, b8) {
...
(function(a) {
	...
	var c = e.cookie = function(p, o, u) {
		if (o !== undefined) {
			...
			return (document.cookie = [c.raw ? p : encodeURIComponent(p), "=", c.raw ? o : encodeURIComponent(o), u.expires ? "; expires=" + u.expires.toUTCString() : "", "; path=" + (u.path ? u.path : "/"), "; domain=" + (u.domain ? u.domain : "meilele.com"), u.secure ? "; secure" : ""].join(""))
		}
		var g = c.raw ? d : b;
...
function static_dynamic(page_sn) {
	...
	if (dom) {
		...
		return goods_id = dom.getAttribute("goods_id"), rgn_id = dom.getAttribute("rgn_id"), debug = dom.getAttribute("debug"), "undefined" != typeof goods_id && goods_id && goods_id > 0 && (url = url + "&goods_id=" + goods_id), "undefined" != typeof rgn_id && rgn_id && rgn_id > 0 && (url = url + "&rgn_id=" + rgn_id), "undefined" != typeof debug && debug && debug > 0 && (url += "&debug=1"), jq.ajax({
			url: url,
			dataType: "json",
			...
			success: function(json) {
				...
				for (var k in json) {
					var callback = null,
						key = json[k].node_name;
					if (key && (tmp[key] = json[k]), json[k].node_name && json[k].html_content.toString().length && document.getElementById(json[k].node_name) && (document.getElementById(json[k].node_name).innerHTML = json[k].html_content), json[k].js_call_func_name) {
						try {
							callback = eval(json[k].js_call_func_name)
						} catch (e) {}
						callback && "function" == typeof callback && callback(json[k])
					}
				}
				window._STATIC_DYNAMIC_CALLBACK && "function" == typeof window._STATIC_DYNAMIC_CALLBACK && window._STATIC_DYNAMIC_CALLBACK(tmp)
			}
		}), !0
	}
}

function _LOAD_VIRTUAL_FUNCTION_(t, e) {
	e ? window[t] = function() {
		return e
	} : window[t] = function() {}
}

function _SET_CHAT_SMS_AD_(t) {
	window._MCHAT_DATA_SMS_AD_ = t
}

function _SET_CHAT_DATA_READY_(t) {
	window._MCHAT_DATA_READY_ = t, __Head_scroll_expr()
}

function __Head_scroll_expr() {
	if (jq("#JS_head_scoll_phone_527").get(0)) {
		window.__region_virtual = window.__region_virtual || jq.cookie("region_virtual"), window.mchat = window.mchat || {}, 1 == window.__region_virtual && jq("#JS_head_scoll_phone_527 .hotLine").html("028-83290176");
		var d = "";
		if (window._MCHAT_DATA_READY_ && window._MCHAT_DATA_READY_.html_content) {
			...
			if (d) {
				mchat.d = d, "function" == typeof d.sort && d.sort(function() {
					return Math.random() - .5
				});
				var hdom = jq("#JS_head_scoll_phone_527");
				if (hdom && d.length) {
					var h = "";
					h += '<div class="expr_scroll">', h += '<div class="expr_view" id="_JS_scroll_phone_g" onmouseover="window._headPhoneScroll527Lock=true"  onmouseout="window._headPhoneScroll527Lock=false">';
					for (var k = 1; 2 >= k; k++) {
						h += '<div class="expr_item" id="JS_show_hot_527"><span>全国热线：</span><span class="hotLine">400 0098666</span></div>';
						for (var i = 0, ii = d.length; ii > i; i++) {
					...
					h += "</div>", h += "</div>";
					var h1 = [];
					1 == d.length ? h1.push('<div class="show_panel_2" id="JS_show_panel_527">') : h1.push('<div class="show_panel" id="JS_show_panel_527">');
					...
					h1.push("</div>"), h += h1.join(""), hdom.html(h), Head_show_phone_527_fn(), window._headPhoneScroll527Dom = hdom.find("#_JS_scroll_phone_g"), window._headPhoneScroll527Length = window._headPhoneScroll527Dom.find("div").length, setInterval(Head_scroll_phone_527_fn, 3e3)
				}
			}
		}
	}
...
function Head_scroll_phone_527_fn() {
	if (!window._headPhoneScroll527Lock) {
		var t = window._headPhoneScroll527Dom,
			e = window._headPhoneScroll527Length,
			i = window._headPhoneScroll527Now;
		t && e && jq(t).animate({
			marginTop: -24 * (i + 1) + "px"
		}, 300, "linear", function() {
			window._headPhoneScroll527Now++, window._headPhoneScroll527Now >= window._headPhoneScroll527Length / 2 && (window._headPhoneScroll527Dom.css("margin-top", "0px"), window._headPhoneScroll527Now = 0)
		})
	}
}

function Head_show_phone_527_fn() {
	var t = jq("#JS_show_panel_527"),
		e = jq("#JS_head_scoll_phone_527");
	jq("#JS_head_scoll_phone_527").hover(function() {
		e.addClass("JS_head_scoll_phone_527_active"), t.show()
	}, function() {
		e.removeClass("JS_head_scoll_phone_527_active"), t.hide()
	})
}
var taobao = 1,
	_LOAD_SCRIPT_ = function(t, e) {
		if (t) {
			if (!e) return void _LOAD_SCRIPT_DELAY_(t);
			var i = document.createElement("script");
			i.type = "text/javascript", i.async = !0, i.src = t, setTimeout(function() {
				document.getElementsByTagName("head")[0].appendChild(i)
			}, 0)
		}
	},
	_G_SCRIPT_lIST_ = [],
	_LOAD_SCRIPT_DELAY_ = function(t) {
		_G_SCRIPT_lIST_[_G_SCRIPT_lIST_.length] = t
	},
	_RUNNING_LOAD_SCRIPT_ = function() {
		if (_G_SCRIPT_lIST_ && _G_SCRIPT_lIST_.length > 0)
			for (var t = 0; t < _G_SCRIPT_lIST_.length; t++) _LOAD_SCRIPT_(_G_SCRIPT_lIST_[t], !0)
	};
window._headPhoneScroll527Now = 0, jq.req = function(t, e) {
	var i = "jq_" + (t + "").replace(/\W/g, "");
	jq.req[i] || (jq.req[i] = jq.ajax({
		cache: !0,
		url: jq.__IMG + "/js/plugin/" + t,
		dataType: "script",
		success: e,
		complete: function() {
			jq.req[i] = !1
...
}, jq.insertStyle = function(t) {
	try {
		style = document.createStyleSheet(), style.cssText = t
	} catch (e) {
		style = document.createElement("style"), style.type = "text/css", style.textContent = t, document.getElementsByTagName("HEAD").item(0).appendChild(style)
	}
}, jq.lightBox = function(t, e, i, n, o, s, a) {
	e = e || {}, n = n || 300;
	var r = document.createElement("div"),
		l = new Date,
		c = l.getTime().toString(36),
		h = !0;
	l = "JS_lightBox_" + c, r.className = "lightBox " + (a ? a : ""), r.id = l, o && (r.style.cssText = o), r.style.zIndex = n;
	var d = '<div class="htmls">' + (t + "").replace(/\{=id\}/g, c) + "</div>",
		_ = "",
		p = 1;
	for (var u in e) _ += '<a href="javascript:;" id="' + l + "_" + u + '" class="lightbox_btns_' + u + " lightbox_btnsi_" + p + '">' + u + "</a>", p++;
	_ && (_ = '<div class="btns">' + _ + "</div>"), d = i ? d + _ : '<div class="in">' + d + _ + "</div>", r.innerHTML = d, r._showMask = s, s && jq.showMask(n - 1), document.body.appendChild(r);
	for (var u in e) {
		var m = jq("#" + l + "_" + u);
		"close" == e[u] ? m[0].onclick = function() {
			jq.closeLightBox(l)
		} : "function" == typeof e[u] && (m[0].onclick = e[u]), h && (m.focus(), h = !1)
	}
	return jq.lightBoxId = l, jq.lightBoxTempId = c, r
}, jq.closeLightBox = function(t, e) {
	t || (t = jq.lightBoxId), t = "JS_lightBox_" + (t + "").replace("JS_lightBox_", "");
	var i = jq("#" + t);
	i.length && (i[0]._showMask && jq.hideMask(), i.remove()), "function" == typeof e && e(t)
}, jq.alert = function(t, e) {
	e = jq.extend({
		title: "提示",
		onOk: null,
		mask: !0,
	...
	var i = '<div class="clearfix title"><div class="Left"><i style="display:inline-block;"></i><span>' + e.title + '</span></div><a href="javascript:;" class="Right" onclick="jq.closeLightBox( \'{=id}\' , jq.alert.setting.onOk );">&times;</a></div><div class="content"><table style="width:100%"><tr><td class="icons_td"><span class="icons icons_' + e.type + '"></span></td><td class="content_td">' + t + "</td></tr></table></div>";
	jq.lightBox(i, {
		"确定": function() {
			jq.closeLightBox(!1, e.onOk)
		}
	}, null, null, null, e.mask, "MALERT"), jq.alert.setting = e
}, jq.confirm = function(t, e) {
	e = jq.extend({
		title: "提示",
		onOk: null,
		onCancel: null,
	...
	var i = '<div class="clearfix title"><div class="Left"><i style="display:inline-block;"></i><span>' + e.title + '</span></div><a href="javascript:;" class="Right" onclick="jq.closeLightBox( \'{=id}\' , jq.confirm.setting.onCancel );">&times;</a></div><div class="content"><table style="width:100%"><tr><td class="icons_td"><span class="icons icons_' + e.type + '"></span></td><td class="content_td">' + t + "</td></tr></table></div>";
	jq.lightBox(i, {
		"确定": function() {
			jq.closeLightBox(!1, e.onOk)
		},
		"取消": function() {
			jq.closeLightBox(!1, e.onCancel)
		}
	}, null, null, null, e.mask, "MALERT MCONFIRM"), jq.confirm.setting = e
}, jq.showMask = function(t) {
	jq._maskDom && jq.hideMask(), t = t || 290;
	var e = document.createElement("div"),
		i = document.body.scrollHeight >= window.screen.availHeight ? document.body.scrollHeight : window.screen.availHeight;
	return e.id = "JS_mask", e.className = "c-mask", e.style.cssText = "zoom:1;background:#000;width:100%;z-index:900;height:" + i + "px;top:0;opacity:.5;z-index:" + t + ";filter:alpha(opacity=50);position:absolute;left:0;", jq._maskDom = jq(e), jq("body").append(jq._maskDom), jq._maskDom
}, jq.hideMask = function() {
	jq._maskDom && (jq._maskDom.remove(), jq._maskDom = null)
}, jq.goExpr = function(t, e, i, n, o) {
	if (t)
		if (1 == o ? jq.cookie("region_virtual", 1, {
				expires: 5,
				domain: "meilele.com"
			}) : jq.cookie("region_virtual", "", {
				expires: -1,
				domain: "meilele.com"
			}), "index.html" == t) jq.cookie("curr_page_sn", "index", {
			expires: 5,
			domain: "meilele.com"
		}), jq.cookie("region_id", 0, {
			expires: 5,
			domain: "meilele.com"
		}), jq.cookie("region_pinyin", "", {
			expires: -1,
			domain: "meilele.com"
		}), jq.cookie("region_name", "全国", {
			expires: 5,
			domain: "meilele.com"
		}), 2 == i ? location.href = "/expr.html" : window._isIndexPage ? location.href = "/" : location.reload();
		else if (jq.cookie("curr_page_sn", 0, {
			expires: 5,
			domain: "meilele.com"
		}), jq.cookie("region_id", e, {
			expires: 5,
			domain: "meilele.com"
		}), jq.cookie("region_pinyin", t, {
			expires: 5,
			domain: "meilele.com"
		}), n ? jq.cookie("region_name", n, {
			expires: 5,
			domain: "meilele.com"
		}) : jq.cookie("region_name", "", {
			expires: -1,
			domain: "meilele.com"
		}), 2 == i) location.href = "/" + t + "/expr.html";
	else {
		var s = location.href.split("/");
		"expr.html" == s[s.length - 1] ? location.href = "/" + t + "/expr.html" : window._isIndexPage ? location.href = "/" + t + "/" : location.reload()
	}
}, jq.getCaptcha = function(t, e) {
	t = t[0] || t, e = e || {};
	var i = {
		url: "/solr_api/captcha/getCaptcha.do?",
		q_url: "/solr_api/captcha/questionCaptcha/getCaptcha.do?"
	};
	t.src = (e.is_q ? i.q_url : i.url) + (e.w ? "w=" + e.w : "") + (e.h ? "&h=" + e.h : "") + "&_=" + Math.random()
}, jq.checkCaptcha = function(t, e, i) {
	t = jq(t[0] || t);
	var n = jq.extend({
		success: function() {},
		error: function() {},
	...
	t.on("keyup blur", function(t) {
		var e = jq(this);
		"blur" == t.type && (e[0]._canError = !0);
		var o = e.val(),
			s = function(t) {
				i && (i = i[0] || i, i.style.background = "url(" + jq.__IMG + "/images/" + t + ".gif) center center no-repeat")
			},
			a = function(t) {
				e[0]._ajax = !1, e[0]._json = t, t && !t.error ? (n.success(t), s("ok")) : (n.error(), s("error"))
			},
			r = function() {
				e[0]._ajax = !1, e[0]._json = !1, s("error")
			};
		if (o == e[0]._lastCaptcha && e[0]._json) return void a(e[0]._json);
		if (o.length == n.charLength) {
			var l = {
				url: "/solr_api/captcha/checkCaptcha.do?",
				q_url: "/solr_api/captcha/questionCaptcha/checkCaptcha.do?"
			};
			e[0]._ajax && e[0]._ajax.abort && e[0]._ajax.abort(), e[0]._lastCaptcha = o, e[0]._canError = !0, e[0]._ajax = jq.ajax({
				url: (n.is_q ? l.q_url : l.url) + "no_clear=1&captcha=" + o,
				cache: !1,
				dataType: "json",
				success: a,
				error: r
			})
		} else e[0]._canError && (n.error(), s("error"))
	})
}, jq.hash = function(t) {
	if (jq.hash._hash && !t) return jq.hash._hash;
	jq.hash._hash = {};
	var e = (location.hash + "").replace("#", "");
	e = e.split("&");
	for (var i = 0, n = e.length; n > i; i++) {
		var o = e[i].split("=");
		jq.hash._hash[o[0]] = o[1]
	}
	return jq.hash._hash
}, jq.setWindowName = function(t) {
	if (t) {
		t = t.toUpperCase();
		var e = Array.prototype.slice.call(arguments, 1);
		window.name += encodeURIComponent("|M#L#L|" + t + "|##|" + e.join("|#|") + "|M#L#L|")
	}
}, jq.getWindowName = function() {
	var t, e, i, n = window.name,
		o = /\|M#L#L\|(.*)\|M#L#L\|/;
	if (n = decodeURIComponent(decodeURIComponent(n)), t = o.exec(n), !t || !(t = t[1])) return !1;
	if (e = t.split("|##|"), "js" === e[0].toLowerCase()) {
		i = e[1].split("|#|");
		try {
			window[i[0]].apply(null, i.slice(1))
		} catch (s) {
			return !1
		}
	}
	n = n.replace(o, ""), window.name = encodeURIComponent(n)
}, jq.tab = function(t, e, i) {
	return t.length <= 0 ? !1 : new jq.tab.prototype.init(t, e, i)
}, jq.tab.prototype = {
	cunstructor: jq.tab,
	init: function(t, e, i) {
		this.tHeads = t || [], this.tBodys = e || [], this.extral = jq.extend({
			lazy: "data-src",
			eType: "mouseover"
		}, i), this.tMore = this.extral.tMore, this.eventType = this.extral.eType, this.execute()
	},
	execute: function() {
		var t = this;
		this.currentIndex = 0, this.tHeads.each(function(e) {
			var i = jq(t.tHeads[e]),
				n = jq(t.tBodys[e]);
			i.length && n.length && (i[0]._Tab = t, i[0]._k = e, i.on(t.eventType, function() {
	...
	setCurrent: function(t) {
		var e = this.tHeads[t],
			i = this.tBodys[t],
			n = this.currentIndex;
		e && i && t != n && (jq(this.tHeads[n]).removeClass("current"), jq(this.tBodys[n]).removeClass("current"), jq(e).addClass("current"), jq(i).addClass("current"), this.tMore && (jq(this.tMore[n]).removeClass("current"), jq(this.tMore[t]).addClass("current"), this.showLazyImg(this.tMore[t])), this.extral && this.extral.lazy && this.showLazyImg(i), this.currentIndex = t)
	},
	showLazyImg: function(t) {
		var e = this,
			i = [];
		t && !t._imgDone && (t = jq(t), jq(t).find("img[" + e.extral.lazy + "]").each(function() {
			var t = jq(this),
				n = t.attr(e.extral.lazy);
			n && i.push({
				target: this,
				src: n
			})
		}), i.length && jq.lazyImg.prepend(i), t._imgDone = !0)
	}
}, jq.tab.prototype.init.prototype = jq.tab.prototype, jq.slide = function(t, e, i) {
	return new jq.slide.prototype.init(t, e, i)
}, jq.slide.prototype = {
	cunstructor: jq.slide,
	init: function(t, e, i) {
		var n = this;
		this.navs = t, this.index = 0, this.navs && this.navs.prevBtn && this.navs.prevBtn && (this.changeBtn = !0), this.stage = e, this.setting = jq.extend({
			type: "margin-left",
			eventType: "click",
			autoRun: !1,
			step: 300,
			onSlide: !1,
			len: !1
		}, i), this.len = this.setting.len || ("margin-left" == this.setting.type ? Math.ceil(this.stage.width() / this.setting.step) : Math.ceil(this.stage.height() / this.setting.step)), this.len <= 1 || (this.changeBtn ? (this.navs.prevBtn.on(this.setting.eventType, function() {
			n.move(n.index - 1)
		}), this.navs.nextBtn.on(this.setting.eventType, function() {
			n.move(n.index + 1)
		})) : this.navs.on(this.setting.eventType, function() {
			var t = jq(this);
			n.move(t.index())
		}), this.setting.autoRun && (this.stage.hover(function() {
			n.lock = !0
		}, function() {
			n.lock = !1
		}), setInterval(function() {
			n.lock || n.move(n.index + 1)
		}, this.setting.autoRun)))
	},
	move: function(t) {
		t = Math.abs(t % this.len);
		var e = {};
		e[this.setting.type] = "-" + t * this.setting.step + "px", this.stage.stop(!0, !1).animate(e), this.changeBtn || (jq(this.navs[this.index]).removeClass("current"), jq(this.navs[t]).addClass("current")), this.index = t, "function" == typeof this.setting.onSlide && this.setting.onSlide(this)
	}
}, jq.slide.prototype.init.prototype = jq.slide.prototype, jq.lazyImg = {
	domList: {},
	waitList: [],
	downList: [],
	status: 0,
	...
	win: jq(window),
	start: function(t, e) {
		this.status > 1 || (this.status = 1, this.type = t, this.setting = jq.extend({
			scrollTimer: 300,
			defaultLimit: 30,
	...
	init: function() {
		this.initDomList(), (!this.type || parseInt(this.type, 10) > 0) && this.setRealType(), "scroll" == this.type || "both" == this.type ? (this.initDomOffsetTop(), this.initEvent(), this.scrollResponse()) : this.prepend(this.domList)
	},
	setRealType: function() {
		this.type || (this.type = this.setting.defaultLimit), parseInt(this.type, 10) > 0 && (this.type = this.domListLength >= this.type ? "both" : "lazy")
	},
	initDomList: function() {
		this.domListLength = 0;
		for (var t, e = document.images, i = 0, n = e.length; n > i; i++) {
			if ("[object Array]" === Object.prototype.toString.apply(this.setting.attributeTag)) t: for (var o = 0, s = this.setting.attributeTag.length; s > o && !(t = e[i].getAttribute(this.setting.attributeTag[o])); o++);
			else t = e[i].getAttribute(this.setting.attributeTag);
			e[i] && t && (this.domList["init_" + i] = {
				target: e[i],
				src: t
			}, this.domListLength++)
		}
	...
	initDomOffsetTop: function() {
		for (var t in this.domList) this.domList[t] && this.domList[t].target && (this.domList[t]._offsetTop = jq(this.domList[t].target).offset().top)
	},
	initEvent: function() {
		this.win.on("scroll", jq.lazyImg.scrollResponse), this.win.on("resize", jq.lazyImg.scrollResponse)
	},
	scrollResponse: function() {
		var t = jq.lazyImg;
		window.scrollTimer && clearTimeout(window.scrollTimer), window.scrollTimer = window.setTimeout(function() {
			var e = window.pageYOffset || document.documentElement.scrollTop || document.body.scrollTop || 0,
				i = [];
			for (var n in t.domList) e + document.documentElement.clientHeight + t.setting.offset > t.domList[n]._offsetTop && e - t.setting.offset < t.domList[n]._offsetTop + (t.domList[n].target.height || t.domList[n].target.cilentHeight) && (i.push(t.domList[n]), delete t.domList[n]);
			t.prepend(i), i = null
		}, t.setting.scrollTimer)
	},
	prepend: function(t) {
		"0" === this.status && this.start(), t = this.formatImglist(t), this.concatList(t, this.waitList)
	},
	append: function(t) {
		"0" === this.status && this.start(), t = this.formatImglist(t), this.concatList(this.waitList, t)
	},
	formatImglist: function(t) {
		t = t || [];
		var e = [];
		for (var i in t) "string" == typeof t[i] && (t[i] = {
			src: t[i]
		}), e.push(t[i]);
		return e
	},
	concatList: function() {
		this.waitList = Array.prototype.concat.apply([], arguments), this.waitList.length > 0 && this.startLoad()
	},
	initLoader: function() {
		this.setting = this.setting || {}, this.loader = [];
		for (var t = 0; t < this.setting.loaderNumber; t++) this.loader.push(new this.Loader(t))
	},
	checkAllLoaderIsReset: function() {
		for (var t = !0, e = 0; e < this.loader.length; e++)
			if ("1" == this.loader[e].status) return !1;
		return t
	},
	startLoad: function() {
		if (!this.loader) return this.initLoader(), void arguments.callee.apply(this);
		for (var t = 0, e = this.loader.length; e > t; t++) this.loader[t].run()
	},
	Loader: function(t) {
		this.key = t, this.run()
	},
	loadPrototype: {
		run: function() {
			1 != this.status && jq.lazyImg.waitList[0] && (this.job = jq.lazyImg.waitList.shift(), this.job && this.job.src ? (this.status = 1, this.img = new Image, this.img.loaderKey = this.key, this.img.onload = this.img.onerror = function() {
				var t = this.loaderKey;
				setTimeout(function() {
					jq.lazyImg.onloadResponse(t)
				}, 0), this.onload = this.onerror = null
			}, this.img.src = this.job.src) : this.run())
		}
	},
	onloadResponse: function(t) {
		var e = jq.lazyImg.loader[t];
		e.job.target && e.job.src && e.job.target.setAttribute("src", e.job.src), e.status = 2;
		var i = !0;
		if ("scroll" == jq.lazyImg.type || "both" == jq.lazyImg.type) {
			var n = [];
			t: for (var o in jq.lazyImg.domList) {
				"both" == jq.lazyImg.type && 0 === jq.lazyImg.waitList.length && (n.push(jq.lazyImg.domList[o]), delete jq.lazyImg.domList[o]), i = !1;
				break t
			}
			Array.prototype.unshift.apply(jq.lazyImg.waitList, n), n = null, i && (this.win.off("scroll", jq.lazyImg.scrollResponse), this.win.off("resize", jq.lazyImg.scrollResponse))
		}
		i && "function" == typeof jq.lazyImg.setting.callback && !jq.lazyImg.waitList[0] && !jq.lazyImg.called && jq.lazyImg.checkAllLoaderIsReset() && (jq.lazyImg.setting.callback(), jq.lazyImg.called = !0), e.run()
	}
}, jq.lazyImg.Loader.prototype = jq.lazyImg.loadPrototype, jq.saleNumUpdate = function(t, e) {
	return new jq.saleNumUpdate.prototype.init(t, e)
}, jq.saleNumUpdate.prototype = {
	constructor: jq.saleNumUpdate,
	defaultSetting: {
		type: "sale_num",
		param: "id",
	...
	init: function(t, e) {
		...
		var i = this;
		this.type = [], this.formatDomList = {}, this.goodsIdString = {}, this.setting = {}, this.callback = e, jq.each(t, function() {
			var t = jq(this.selector),
				e = [],
				n = {};
			t.each(function(t, i) {
				var o = jq(this),
					s = o.data("goods_id");
				jq.isNumeric(s) && (n[s] ? jq.merge(n[s], o) : (n[s] = jq.merge(jq(), o), e.push(s)))
			}), i.type.push(this.type), i.formatDomList[this.type] = n, e.length > 0 && (i.goodsIdString[this.type] = "_" + e.join("_") + "_", i.flag = !0), i.setting[this.type] = this, this.isInt && (i.setting.isInt = 1)
	...
	start: function() {
		...
		if (jq.each(this.type, function(i, n) {
				e[t.setting[n].param] = t.goodsIdString[n]
			}), e.price_id && delete e.price_id, e.id)
			for (var i = e.id.split("_"), n = [], o = 0; o < i.length; o++) parseInt(i[o]) > 0 && n.push(parseInt(i[o]).toString(36));
		n && n.length && (e.id = "_" + n.join("_"), e.ids = e.id, delete e.id, this.setting.isInt && (e.isInt = this.setting.isInt), jq.ajax({
			url: "/dubbo_api/realtimeSite/get?n=1",
			type: "get",
			data: e,
			dataType: "json",
			cache: !0,
			success: function(e) {
				e && 0 == e.error && e.result && (jq.each(t.type, function(i, n) {
					var o = t.setting[n].key,
						s = t.formatDomList[n];
					e.result[n] && jq.each(e.result[n], function() {
						var t = this.goods_id,
							e = this[o],
							i = s[t];
						i && i.length && e && i.html(e)
					})
				}), t.callback && "function" == typeof t.callback && t.callback())
			}
		}))
	}
}, jq.saleNumUpdate.prototype.init.prototype = jq.saleNumUpdate.prototype, jq.addGoodsToCollect = function(t, e, i) {
	jq.ajax({
		url: "/add_cart/?step=insertGoodsToCollect&goodsId=" + t,
		dataType: "json",
		cache: !1,
		success: function(t) {
			i && i(), 0 == t.error ? e ? e(t) : jq.alert("收藏成功") : 2 == t.error ? jq.alert("请登录后再进行收藏！") : 3 == t.error ? jq.alert("该商品已经存在于您的收藏夹中。") : jq.alert(t.msg)
		},
		error: function() {
			i && i(), jq.alert("由于网络原因，收藏失败，请稍后再试。")
		}
	})
}, jq.localData = function(t, e, i) {
	function n(t, e) {
	...
	function o(t) {
		var e = 0,
			...
			a = s.split("%%");
		return e = a[0], o = n - e, o > 60 * i * 60 * 1e3 && (this.removeLocalData(), a = null), s = /^\{.*\}jq/.test(a[1]) ? JSON.parse(a[1]) : a[1]
	}

	function s(t) {
		localStorage.removeItem(t)
	}
	jq.isSupportLocalStorage = !1, window.localStorage && window.JSON && (jq.isSupportLocalStorage = !0), jq.isSupportLocalStorage && t && (i = i || 72, void 0 !== e ? -1 === e ? s(t) : n(t, e) : o(t))
}, jq(function() {
	if (window._onReadyList && window._onReadyList.length)
		for (var t = 0, e = _onReadyList.length; e > t; t++) jq.isFunction(_onReadyList[t]) && _onReadyList[t]();
	jq.getWindowName();
	var i = jq.hash();
	if (i && i.MLLJS) {
		i = i.MLLJS.split("||");
...
}), window.addEventListener ? window.addEventListener("load", _RUNNING_LOAD_SCRIPT_, !1) : window.attachEvent ? window.attachEvent("onload", _RUNNING_LOAD_SCRIPT_) : window.onload = _RUNNING_LOAD_SCRIPT_;
